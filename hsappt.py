...
def open_appointment_window():
    ...
    #doctor_label = doctor_var.get()
    #slot_var.set("")  # Clear the currently selected slot
    #slot_option_menu['menu'].delete(0, 'end')  # clear all options

    #if doctors in doctors:
        #for slot in doctors[doctors]:
            #slot_option_menu['menu'].add_command(label=slot, command=tk._setit(slot_var, slot))
        #slot_var.set(doctors[doctors][0])  # set the default slot to the first available slot
    ...
    slot_var.set(doctors[doctor_var.get()][1])
    # default slot based on selected doctor // wait lang here
